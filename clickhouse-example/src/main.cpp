...
int main() {
  ...
  const ObjectWithUlid a{
    OrderBook{
      .level = 1
    }
  };
  const ObjectWithUlid b{
    OrderBook{
      .level = 2
    }
  };

  std::cout << a << ' ' << hash_value(a) << '\n';
  std::cout << b << ' ' << hash_value(b) << '\n';
}
...
//#include <mdh/dvc/core_price_filtered.h>
//#include <mdh/dvc/fads_markup.h>
//#include <mdh/dvc/forward_points_blending_weight.h>
//#include <mdh/dvc/ilp_trades.h>
//#include <mdh/dvc/manual_forward_points.h>
//#include <mdh/dvc/order_book.h>
//#include <mdh/dvc/order_book_wide.h>
//#include <mdh/dvc/position_transfer_service_data.h>
...
//
//  const auto args = std::make_shared<Args>(logger, "config.toml");
//
//  if (!args) { _Exit(EXIT_FAILURE); }
//
